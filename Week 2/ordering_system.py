...
def calculate_subtotal(order):
    ...
    # Add every item in the order to the subtotal.
    for item in order:
        subtotal += item['price']
...
def calculate_tax(subtotal):
    ...
    # Calculating tax, rounding it to two dec. and returning it.
...
def summarize_order(order):
    ...
    # Calculate the total and rounding it to two decimals.
    subtotal = calculate_subtotal(order)
    tax = calculate_tax(subtotal)
    total = round((subtotal + tax), 2)

    # Creating a list of names of all the items in the order.
